...
<head>
    ...
    <title>NexTugas — Sign In</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <!-- Konfigurasi Tailwind CSS agar mengenali class 'dark' untuk Dark Mode -->
...
<!-- 
    CSS PRESENTATION TRICKS:
    - w-full h-screen: Lock body size to full monitor screen (100vh).
    - overflow-hidden: Hide decorative elements that go out of bounds to prevent scrollbars.
    - flex items-center justify-center: Place the login card precisely in the center vertically & horizontally.
-->
<body class="bg-zinc-50 dark:bg-zinc-950 w-full h-screen overflow-hidden flex items-center justify-center relative transition-colors duration-300">
    ...
    {{-- Main Login Card: 2 panels (Left Form + Right Illustration) --}}
    <div class="relative z-10 w-full max-w-4xl flex rounded-3xl shadow-2xl shadow-zinc-200/80 dark:shadow-black/40 border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900 mx-4 overflow-hidden min-h-[500px]">

        {{-- === LEFT PANEL: Login Form === --}}
        {{-- Uses solid white background in Light Mode, changes to semi-transparent Glassmorphism in Dark Mode --}}
        <div class="w-full md:w-[45%] p-8 md:p-10 flex flex-col justify-center bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md">
            ...
            <p class="text-xs text-zinc-400 mb-6">Please sign in to manage your school tasks.</p>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-3">
                ...
                <div class="flex items-center justify-between py-0.5">
                    <label class="flex items-center gap-1.5 cursor-pointer text-[11px] text-zinc-500 dark:text-zinc-400">
                        <input type="checkbox" name="remember" class="rounded border-zinc-300 dark:border-zinc-600 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-0 h-3.5 w-3.5 bg-zinc-50 dark:bg-zinc-800">
                        Remember me
                    </label>
                    <a href="#" class="text-[11px] text-zinc-400 hover:text-indigo-500 transition-colors">Forgot Password?</a>
                </div>
            ...
            <p class="mt-4 text-center text-xs text-zinc-400">
                Don't have an account? <a href="{{ route('register') }}" class="text-indigo-500 hover:text-indigo-400 font-medium transition-colors">Sign Up</a>
            </p>
