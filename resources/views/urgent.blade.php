...
@section('page-title', 'Urgent Tasks')
@section('page-subtitle', 'Tasks requiring immediate attention')
...
<div x-data="{ activeSubject: 'All', tasks: {{ $alpineTasks }} }" class="flex-1 h-full overflow-y-auto p-4 md:p-6 w-full flex flex-col gap-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-red-500 dark:text-red-400 mt-1 flex items-center gap-1.5 font-medium">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
                {{ $tasks->count() }} {{ Str::plural('task', $tasks->count()) }} nearing deadline
            </p>
        </div>
    ...
    <div class="flex items-center gap-6 border-b border-zinc-200 dark:border-zinc-800/80 overflow-x-auto hide-scrollbar">
        ...
        @foreach($tabs as $tab)
            <button @click="activeSubject = '{{ $tab }}'"
                    :class="activeSubject === '{{ $tab }}' ? 'border-red-500 text-red-600 dark:text-red-400 font-semibold' : 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300'"
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800/50 rounded-2xl overflow-hidden flex flex-col shadow-sm">
            ...
            <div x-show="activeSubject === 'All' || activeSubject === '{{ $task->category ?: 'Others' }}'"
                ...
                <div class="flex items-center space-x-4 min-w-0">
                    <div class="relative z-20 flex items-center justify-center">
                        <div class="w-5 h-5 rounded-full border-2 border-red-400 dark:border-red-500"></div>
                    </div>

                    <div class="min-w-0 flex flex-col">
                        <a href="{{ route('tasks.show', $task->id) }}" class="font-medium text-zinc-900 dark:text-white hover:text-indigo-600 transition-colors after:absolute after:inset-0 after:z-10 truncate">
                            {{ $task->title }}
                        </a>
                ...
                <div class="relative z-20 flex items-center space-x-3 shrink-0">
                    <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400">
                    ...
                    <form id="form-delete-urgent-{{ $task->id }}" action="{{ route('tasks.destroy', $task) }}" method="POST" class="inline-flex">
                        @csrf @method('DELETE')
                        <button type="button" title="Delete Task"
                                @click.prevent="$dispatch('open-confirm', { title: 'Delete Urgent Task', message: 'Are you sure you want to delete this urgent task?', confirmText: 'Delete', type: 'danger', formId: 'form-delete-urgent-{{ $task->id }}' })"
        ...
        @empty
            <div class="border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl p-8 text-center flex flex-col items-center justify-center min-h-[300px] bg-white dark:bg-zinc-900/50 m-4">
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs mt-1">You don't have any urgent tasks at the moment.</p>
        ...
        @if($tasks->count() > 0)
            ...
            <div x-show="tasks.filter(t => activeSubject === 'All' || activeSubject === t.category).length === 0" 
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs mt-1">You haven't created any urgent tasks for this subject yet.</p>
